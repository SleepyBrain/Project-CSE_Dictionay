...
//				Log.d("DICTIONARY All", wordDefinition.word+" "+wordDefinition.definition);
	...
	//For doing auto completation in edittext

	public ArrayList<String> getWords() {
		ArrayList<String> arrayList = new ArrayList<String>();
		SQLiteDatabase database = this.getReadableDatabase();
		String selectWordColumn = "SELECT " + WORD_COLUMN + " FROM " + DICTIONARY_DATABASE;
		Cursor cursor = database.rawQuery(selectWordColumn, null);
		if (cursor.moveToFirst()) {
			do {
				String string = new String(cursor.getString(cursor.getColumnIndex(WORD_COLUMN)));
//				Log.d("DICTIONARY", string);
